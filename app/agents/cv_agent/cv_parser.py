...
# BUG-18 FIX : noms de mois FR/EN extraits comme company
# Ex : "Mars 2023 - Présent" → company="Mars"
MONTHS_NAMES_BLACKLIST = {
    # Français
    ...
    # Abréviations
    "jan", "fev", "feb", "mar", "apr", "avr", "jun", "jul", "juil",
    "aug", "aou", "sep", "sept", "oct", "nov", "dec",
}

# BUG-21 FIX : titres de diplômes extraits comme company de stage
DEGREE_KEYWORDS_BLACKLIST = {
    "licence", "license", "master", "bts", "bachelor", "ingénieur", "ingenieur",
    "doctorat", "doctorate", "phd", "dut", "but", "deug", "capacite",
    "diplôme", "diplome", "formation", "cursus", "école", "ecole", "university",
    "université", "universite", "iset", "esprit", "insat", "enit", "ensi",
    "isimg", "isitcom", "fst", "fseg", "isi", "isg",
}
...
def fix_city_as_company(result: dict) -> dict:
    ...
    for key in ["professional_experience", "internships", "alternance"]:
        for exp in (result.get(key) or []):
            ...
            # BUG-18 FIX : company est un nom de mois (ex: "Mars" depuis "Mars 2023 - Présent")
            if company_lower in MONTHS_NAMES_BLACKLIST:
                logger.info(
                    f"  [fix-city] company='{company}' → None "
                    f"('{exp.get('role')}' — nom de mois détecté comme entreprise)"
                )
                exp["company"] = None
                continue

            # BUG-21 FIX : company est un titre de diplôme (ex: "Licence Informatique")
            # Cas : stage sans nom d'entreprise → parser prend le diplôme le plus proche
            words_company = company_lower.split()
            if words_company and words_company[0] in DEGREE_KEYWORDS_BLACKLIST:
                logger.info(
                    f"  [fix-city] company='{company}' → None "
                    f"('{exp.get('role')}' — titre de diplôme détecté comme entreprise)"
                )
                exp["company"] = None
                continue
...
def extract_period_from_duration(
    ...
    # BUG-12 FIX : utiliser mois=6 (milieu d'année) pour les bornes
    # au lieu de mois=1 pour les deux → réduit l'inflation artificielle
    # Explication : "2020 - 2022" est ambigu sur les mois exacts.
    # Mois=1 pour début ET mois=12 pour fin surestimait systématiquement.
    # Mois=6 pour les deux donne une estimation centrée et plus juste.
    elif len(years_found) >= 2:
        start_year = years_found[0]
        end_year   = CURRENT_YEAR if present else years_found[-1]
        end_month  = CURRENT_MONTH if present else 6
        return start_year, 6, end_year, end_month
...
def classify_experience_weighted(
    ...
    # ── P1.5 BUG-14 FIX : durée > 12 mois → professionnel forcé ───────
    # Un stage ne dure jamais plus de 12 mois en contexte tunisien/français.
    # Si la durée calculée dépasse 12 mois → c'est un poste professionnel,
    # peu importe le titre (assistante, junior, etc.)
    # Cas visé : "Assistante Administrative 2023–Présent" (36+ mois)
    sy_pre, sm_pre, ey_pre, em_pre = extract_period_from_duration(exp.get("duration", ""))
    if sy_pre is not None:
        dur_pre = max((ey_pre - sy_pre) * 12 + (em_pre - sm_pre), 0)
        if dur_pre > 12:
            logger.info(
                f"  → PRO    [P1.5 BUG14 conf=0.90] durée={dur_pre}mois>12 : '{exp.get('role')}'"
            )
            return "professional_experience", 0.90
...
"""
    ...
    for attempt in range(3):
        try:
            ...
            # BUG-06 FIX : si languages=[] après toutes les passes → inférer depuis le texte
            # Un CV rédigé en français implique au minimum que le candidat parle français
            if not result.get("languages"):
                inferred_langs = []
                text_for_lang  = (raw_ocr_text or "").lower()
                # Marqueurs FR forts
                fr_markers = ["je ", "mon ", "ma ", "mes ", "nous ", "vous ",
                              "expérience", "experience", "compétences", "competences",
                              "formation", "poste", "entreprise", "cordialement"]
                en_markers = ["i am", "my ", "experience", "skills", "education",
                              "company", "sincerely", "regards", "dear "]
                fr_score = sum(1 for m in fr_markers if m in text_for_lang)
                en_score = sum(1 for m in en_markers if m in text_for_lang)
                if fr_score >= 3:
                    inferred_langs.append("Français")
                if en_score >= 3:
                    inferred_langs.append("Anglais")
                if inferred_langs:
                    result["languages"] = inferred_langs
                    logger.info(
                        f"  BUG-06 FIX : languages=[] → inféré depuis texte CV : {inferred_langs}"
                    )
